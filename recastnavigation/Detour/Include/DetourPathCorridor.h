...
public:
	/**
	 * @brief Constructor for dtPathCorridor.
	 */
	dtPathCorridor();
	/**
	 * @brief Destructor for dtPathCorridor.
	 */
	~dtPathCorridor();
	...
	/**
	 * @brief Moves the agent over an off-mesh connection.
	 *
	 * This function is used to move the agent over an off-mesh connection, updating the path accordingly.
	 *
	 * @param[in] offMeshConRef   The reference to the off-mesh connection polygon.
	 * @param[out] refs           An array to store the new polygon references along the path.
	 * @param[in,out] startPos    The start position for the move operation. Updated with the new position.
	 * @param[in,out] endPos      The end position for the move operation. Updated with the new position.
	 * @param[in] navquery        The query object used to build the corridor.
	 * @return True if the move operation succeeded.
	 */
	...
	/**
	 * @brief Fixes the start of the path to the specified safe polygon and position.
	 *
	 * This function adjusts the start of the path to a safe position within the specified polygon,
	 * ensuring that the agent's path starts from a valid location.
	 *
	 * @param[in] safeRef         The reference to the safe polygon.
	 * @param[in] safePos         The safe position within the safe polygon. [(x, y, z)]
	 * @return True if the path start was successfully fixed.
	 */
	bool fixPathStart(dtPolyRef safeRef, const float* safePos);

	/**
	 * @brief Trims an invalid path from the end, starting from the specified safe polygon and position.
	 *
	 * This function trims an invalid path from the end, starting from the specified safe polygon and position.
	 * It ensures that the agent's path remains valid and does not enter invalid areas.
	 *
	 * @param[in] safeRef         The reference to the safe polygon.
	 * @param[in] safePos         The safe position within the safe polygon. [(x, y, z)]
	 * @param[in] navquery        The query object used to build the corridor.
	 * @param[in] filter          The filter to apply to the operation.
	 * @return True if the path was successfully trimmed.
	 */
...
/**
 * @brief Merges the corridor path after its start has been moved.
 *
 * This function is used to merge the corridor path after its start has been moved to a new location.
 * It ensures that the path remains continuous and valid.
 *
 * @param[in,out] path      The corridor path to be merged. Modified to reflect the merged path.
 * @param[in] npath         The number of polygons in the corridor path.
 * @param[in] maxPath       The maximum number of polygons the corridor path can hold.
 * @param[in] visited       An array of visited polygon references to assist in the merge.
 * @param[in] nvisited      The number of visited polygon references.
 * @return The new number of polygons in the merged corridor path.
 */
int dtMergeCorridorStartMoved(dtPolyRef* path, const int npath, const int maxPath,
	const dtPolyRef* visited, const int nvisited);

/**
 * @brief Merges the corridor path after its end has been moved.
 *
 * This function is used to merge the corridor path after its end has been moved to a new location.
 * It ensures that the path remains continuous and valid.
 *
 * @param[in,out] path      The corridor path to be merged. Modified to reflect the merged path.
 * @param[in] npath         The number of polygons in the corridor path.
...
/**
 * @brief Merges the corridor path after its start has been shortcutted.
 *
 * This function is used to merge the corridor path after its start has been shortcutted.
 * It ensures that the path remains continuous and valid.
 *
 * @param[in,out] path      The corridor path to be merged. Modified to reflect the merged path.
 * @param[in] npath         The number of polygons in the corridor path.
